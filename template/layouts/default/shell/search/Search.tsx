...
export function Search(): ReactElement {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      const dbResponse = await fetch(`${import.meta.env.BASE_URL}assets/oramaDB_pages.json`);
      if (dbResponse.ok) {
  ...
  useEffect(() => {
    if (pagesDb && debouncedQuery) {
      const results: Results<PageSchema> | Promise<Results<PageSchema>> = search(pagesDb, {
        term: debouncedQuery,
      });
        ...
        <div className="dba-search-result-container">
          ...
          {(searchResults?.hits?.length || 0) > 0 ? (
            searchResults?.hits.map((hit) => {
              ...
              const { start, termIndex, end } = getMarkedContent(content, query);
              return (
                <a
                ...
                >
                  <DBCard>
                    ...
                    <small>
                      ...
                      <mark>{content.substring(termIndex, termIndex + query.length)}</mark>
