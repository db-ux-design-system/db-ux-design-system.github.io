(function () {
	...
	function initRoot(root) {
		...
		const optionButtons = Array.from(root.querySelectorAll('[data-image-toggle-button]')).filter(
			(n) => n instanceof HTMLElement,
		);

		const modeButtons = Array.from(root.querySelectorAll('[data-image-toggle-to]')).filter(
			(n) => n instanceof HTMLElement,
		);
		...
		const getActiveId = () =>
			root.getAttribute('data-active-id') || optionButtons[0]?.getAttribute('data-image-id') || '';
		...
		const updateModeButtons = () => {
			...
			modeButtons.forEach((btn) => {
				...
				const show = (mode === 'light' && to === 'dark') || (mode === 'dark' && to === 'light');
